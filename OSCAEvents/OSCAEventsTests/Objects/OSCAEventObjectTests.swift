...
///  | pygmentize -g
///  ```
class OSCAEventObjectTests: OSCAParseClassObjectTestSuite<OSCAEvent> {
  override open func makeSpecificObject() -> OSCAEvent? {
    nil
  ...
  /// The file name has to match with the test class name: `OSCAEventObjectTests.json`
  override func testJSONDataAvailable() throws -> Void {
    try super.testJSONDataAvailable()
  }// end override func testJSONDataAvailable
#if DEBUG
  /// Is it possible to deserialize `JSON` scheme example data to an array  `OSCAEvent` 's?
  override func testDecodingJSONData() throws -> Void {
    try super.testDecodingJSONData()
  }// end override func testDecodingJSONData
#endif
  /// is it possible to fetch up to 1000 `OSCAEvent` objects in an array from network?
...
}// end class OSCAEventObjectTests
#endif
